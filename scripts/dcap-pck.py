...
def validate_platform(platform: dict[str, Any]) -> dict[str, str]:
    ...
    if normalized["enc_ppid"] and not re.fullmatch(
        r"[0-9a-f]+", normalized["enc_ppid"]
    ):
...
def create_manifest(
    bundle_type: str, files: dict[str, Path], **extra: Any
) -> dict[str, Any]:
    return {
...
def validate_manifest(bundle: Path, expected_type: str) -> dict[str, Any]:
    ...
    if (
        ...
        or manifest.get("type") != expected_type
    ):
...
def cluster_platforms(namespace: str) -> list[dict[str, str]]:
    ...
    for secret in data.get("items", []):
        ...
        try:
            decoded = {
                field: base64.b64decode(encoded[field]).decode("ascii")
                for field in PLATFORM_FIELDS
            }
        except (KeyError, UnicodeDecodeError, ValueError) as error:
            name = secret.get("metadata", {}).get("name", "<unknown>")
            fail(f"invalid platform-data Secret {name}: {error}")
...
def command_export(arguments: SimpleNamespace) -> None:
    ...
    write_json(
        ...
        create_manifest(
            REQUEST_TYPE,
            {"platform-list.json": platform_list},
            namespace=arguments.namespace,
            platforms=[
                {"qe_id": item["qe_id"], "pce_id": item["pce_id"]} for item in platforms
            ],
        ),
    )
...
def read_api_key() -> str:
    ...
    if not api_key:
        if not sys.stdin.isatty():
            fail(
                "INTEL_PCS_API_KEY is not set and no interactive terminal is available"
            )
...
def generate_cache(
    tool_dir: Path, platforms_file: Path, output: Path, expire_hours: int, api_key: str
) -> None:
...
def command_generate(arguments: SimpleNamespace) -> None:
    ...
    try:
        ...
        generate_cache(
            Path(arguments.pcsclient_dir).expanduser(),
            request / "platform-list.json",
            cache_dir,
            arguments.expire_hours,
            api_key,
        )
        ...
        if generated != expected:
            message = (
                f"generated PCK cache does not match request; "
                f"expected {sorted(expected)}, got {sorted(generated)}"
            )
            fail(message)
        ...
        write_json(
            ...
            create_manifest(
                RESPONSE_TYPE,
                files,
                requestSha256=sha256(request / "manifest.json"),
                platforms=platforms,
                cache=metadata,
            ),
        )
...
def response_cache(
    bundle: Path,
) -> tuple[dict[str, Any], list[dict[str, str]], dict[tuple[str, str], Path]]:
...
def qgs_daemonset(namespace: str, configured_name: str) -> str:
    ...
    pods = json.loads(run_oc(["get", "pods", "-n", namespace, "-o", "json"])).get(
        "items", []
    )
    names = set()
    for pod in pods:
        ...
        if not any(
            container.get("name") == "pck-certs-watcher"
    ...
    if len(names) != 1:
        fail(
            "expected exactly one QGS DaemonSet from pck-certs-watcher pod owners; "
            "set DCAP_QGS_DAEMONSET to override"
        )
    return names.pop()
...
def command_import(arguments: SimpleNamespace) -> None:
    ...
    if current != expected:
        fail(
            "response bundle platform data does not match current platform-data Secrets"
        )
    ...
    digest = sha256(bundle / "manifest.json")
    print(f"Imported {len(cache)} PCK cache Secrets from {bundle} (SHA-256: {digest})")
...
def command_provision(arguments: SimpleNamespace) -> None:
    ...
    if request.exists():
        ...
        if exported == current:
            ...
            command_export(
                SimpleNamespace(namespace=arguments.namespace, output=str(request))
            )
    else:
        command_export(
            SimpleNamespace(namespace=arguments.namespace, output=str(request))
    ...
    if response.exists():
        ...
        if (
            response_manifest.get("requestSha256") == request_digest
            and response_platforms == current
        ):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    tools.add_argument(
        "--repository",
        default=os.environ.get("DCAP_PCSCLIENT_REPO", "~/.coco-pattern/intel-dcap"),
    )
    tools.add_argument(
        "--pcsclient-dir",
        default=os.environ.get(
            "DCAP_PCSCLIENT_DIR", "~/.coco-pattern/intel-dcap/tools/PcsClientTool"
        ),
    )
    tools.add_argument(
        "--ref",
    ...
    export = commands.add_parser(
        "export", help="export QGS platform data from the disconnected cluster"
    )
    export.add_argument(
        "--namespace",
        default=os.environ.get("DCAP_NAMESPACE", "intel-dcap-operator-system"),
    )
    export.add_argument(
        "--output",
        default=os.environ.get(
            "DCAP_REQUEST_BUNDLE", "~/.coco-pattern/dcap-pck/platform-request"
        ),
    )
    export.set_defaults(func=command_export)
    generate = commands.add_parser(
        "generate", help="generate a PCK response bundle on the connected low side"
    )
    ...
    generate.add_argument(
        "--output",
        default=os.environ.get(
            "DCAP_RESPONSE_BUNDLE", "~/.coco-pattern/dcap-pck/pck-response"
        ),
    )
    generate.add_argument(
        "--pcsclient-dir",
    ...
    generate.add_argument(
        "--expire-hours",
        type=expire_hours,
        default=os.environ.get("DCAP_PCK_EXPIRE_HOURS", "8760"),
        metavar="HOURS",
    )
    generate.set_defaults(func=command_generate)
    importer = commands.add_parser(
        "import", help="import a PCK response bundle into the disconnected cluster"
    )
    ...
    importer.add_argument(
        "--qgs-daemonset", default=os.environ.get("DCAP_QGS_DAEMONSET", "")
    )
    importer.add_argument(
        "--timeout", default=os.environ.get("DCAP_QGS_TIMEOUT", "10m")
    )
    importer.add_argument(
        "--input",
        default=os.environ.get(
    ...
    provision = commands.add_parser(
        "provision", help="resume the connected-bastion PCK lifecycle"
    )
