...
export PATTERN_NAME="coco-pattern"
...
if [ -z "${WORKSPACE}" ]; then
    export WORKSPACE=`mktemp -d` # let the OS decide
fi

pytest -lv --disable-warnings test_subscription_status_simple.py --kubeconfig $KUBECONFIG --junit-xml $WORKSPACE/test_subscription_status_simple.xml

pytest -lv --disable-warnings test_validate_components_simple.py --kubeconfig $KUBECONFIG --junit-xml $WORKSPACE/test_validate_components_simple.xml
